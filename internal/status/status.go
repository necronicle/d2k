...
func Collect(c config.Config, startedAt time.Time) Snapshot {
	...
	s.Stages = []Stage{
		...
		{Key: "capture", Title: "Чтение пакетов", Built: false,
			Detail: "Датапат не написан. Очередь ядра не открывается, ни один пакет не читается."},
		{Key: "flow", Title: "Состояние потоков", Built: false,
			Detail: "Учёта соединений нет: считать нечего, пока пакеты не читаются."},
		{Key: "plan", Title: "Исполнение планов", Built: false,
			Detail: "Формат плана не определён, исполнителя нет. Обход не применяется ни к одному соединению."},
		{Key: "boxes", Title: "Каталог коробок", Built: false,
			Detail: "Хранилища нет. Ни одна модель DPI не изучена и не сохранена."},
		{Key: "panel", Title: "Панель", Built: true,
			Detail: "Показывает этот снимок. Только чтение: изменяющих запросов у неё пока нет."},
	}

	s.Absent = []Stage{
		{Key: "traffic", Title: "Наблюдаемый трафик",
			Detail: "Нечем: чтения пакетов нет. Это не «ноль соединений», а отсутствие измерения."},
		{Key: "bypass", Title: "Обойдённые цели",
			Detail: "Нечем: исполнителя планов нет. Пустой список здесь означал бы, что обход работает и никому не понадобился."},
		{Key: "searches", Title: "Идущие поиски",
			Detail: "Нечем: планировщика нет."},
		{Key: "outages", Title: "Здоровье наблюдения",
			Detail: "Нечем: нечего наблюдать, поэтому и о здоровье наблюдения сказать нечего."},
	}
	return s
}
