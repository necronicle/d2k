...
func TestОтсутствующиеПоказанияНеПоказаныНулями(t *testing.T) {
	...
	// Без подключения к датапату отсутствует именно НАБЛЮДЕНИЕ, и панель
	// обязана назвать это отсутствием измерения, а не нулём. Обойдённые цели
	// в этот список больше не входят: они считаются по каталогу, и когда
	// каталог открыт, их число — измерение, а не дырка в приборе.
	if !strings.Contains(body, "Наблюдаемый трафик") {
		t.Error("отсутствие наблюдения не названо")
	}
	if !strings.Contains(body, "отсутствие измерения") {
		t.Error("отсутствие измерения не отличено от нуля")
	}
}
