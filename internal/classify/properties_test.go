...
func TestЛабораторияВсёСразуИсполняетТриПриёмаВместе(t *testing.T) {
	...
	// Тот же 20-байтовый сценарий и тот же sni_off=2/sni_len=4 (середина
	// имени = 2+4/2 = 4), что и у TestЛабораторияПорядокШлётТриКускаПоИмени
	// (reorder-cut): резы в 1 и 4 дают три куска [0,1), [1,4), [4,20) —
	// голова длиной 1 (сдвиг -1 из-за приставки перекрытия), середина
	// длиной 3, хвост длиной 16. Числа проверены planlab'ом вручную перед
	// тем, как их здесь закрепить.
	out := runPlanLab(t, cp.Text, "pkt 1000 2 4 aabbccddeeff00112233445566778899aabbccdd\n")
	if strings.HasPrefix(out, "reject") {
		t.Fatalf("план «всё сразу» отвергнут: %s", out)
	}
	lines := strings.Split(strings.TrimSpace(out), "\n")
	if len(lines) != 6 { // 2 фальшивки + хвост + середина + голова + fate
		t.Fatalf("строк %d, ожидалось 6 (2 фальшивки, 3 куска, fate): %s", len(lines), out)
	}

	fake0, fake1 := strings.Fields(lines[0]), strings.Fields(lines[1])
	tail, mid, head := strings.Fields(lines[2]), strings.Fields(lines[3]), strings.Fields(lines[4])
	...
	if tail[1] != "payload" || mid[1] != "payload" || head[1] != "payload" {
		t.Fatalf("после фальшивок ожидались три куска нагрузки: %v / %v / %v", tail, mid, head)
	}
	// Обратный порядок переставляет только КРАЙНИЕ куски — хвост [mid,n) и
	// голову [0,1); средний [1,mid) остаётся на своей позиции (plan_apply.c
	// меняет местами только первый и последний PAYLOAD-кусок, см.
	// datapath/test_plan_apply.c). Приставка перекрытия стоит на куске, что
	// при построении был первым (голова), и переворот её не переносит —
	// меняется ОТПРАВКА, а не то, к какому куску приставлена приставка.
	if tail[3] != "1004" {
		t.Errorf("хвост с номером %s, ожидался 1004 (1000+4, середина имени при sni_off=2,sni_len=4)", tail[3])
	}
	if mid[3] != "1001" {
		t.Errorf("середина с номером %s, ожидался 1001 (1000+1) — переворот её не касается", mid[3])
	}
	// Голова уходит ПОСЛЕДНЕЙ (а не второй, как при двух кусках до фикса) и
	// несёт приставку перекрытия — номер сдвинут назад на длину приставки
	// (1 байт).
	if head[3] != "999" {
	...
	if lines[5] != "fate drop" {
		t.Errorf("fate %q, ожидался drop — нагрузку выпустили сами", lines[5])
	}
}
