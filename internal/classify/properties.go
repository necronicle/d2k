...
func Compose(pr Properties, decoy string) ([]catalog.Plan, error) {
	...
	// Порядок — из task-4-plans.md, раздел «Порядок вопросов»: «сперва то,
	// что прямо следует из измеренного (перекрытие, порядок, серия)». Этот
	// порядок СВОЙ, отличный от порядка опроса в PropProbes — там он
	// подчинён цене таймаута, а не приоритету готового кандидата.
...
// собраны перекрытие слева и разнесённая пара дублей ВМЕСТЕ, одним планом —
// оба вместе проверены planlab'ом (properties_test.go) на связке
// «seqovl+fake без разреза», которую datapath/test_plan_apply.c покрывает
// по частям (перекрытие отдельно, фальшивка отдельно).
//
// Порядок сегментов (split+reverse) сюда намеренно НЕ входит: комбинация
// «order reverse ОДНОВременно с фальшивками» различает фальшивку и нагрузку
// по адресу буфера (datapath/plan_apply.c), а не по метке, и ни один тест
// исполнителя эту связку не проверяет. Заводить непроверенную комбинацию в
// кандидат по умолчанию, который поедет на боевой путь при КАЖДОМ пустом
// векторе, — не тот случай, где такой риск оправдан; вопрос о порядке
// сегментов при этом никуда не делся — он останется своим отдельным
// кандидатом, как только измерение его подтвердит (см. Compose).
