...
func (o *Options) withDefaults() {
	if o.Repeats <= 0 {
		// ТРИ, А НЕ ОДИН, И ВЕРДИКТ ТОЛЬКО ПРИ ЕДИНОГЛАСИИ.
		//
		// Один повтор я поставил, гонясь за пятнадцатью секундами. Это было
		// ошибкой в постановке: инструмент, которому нельзя верить, не нужен
		// ни за пятнадцать секунд, ни за две минуты. Донор тратит до двух
		// минут и требует совпадения всех трёх — и потому его вердикт
		// переносится на живой трафик, а мой не переносился.
		o.Repeats = 3
	}
	if o.Gap <= 0 {
		// ШЕСТЬДЕСЯТ МИЛЛИСЕКУНД МЕЖДУ КУСКАМИ, В БОЕВОМ УМОЛЧАНИИ.
		//
		// Я ставил здесь ноль и отдельно доказывал, что пауза нужна только на
		// петле как артефакт стенда. Это опровергается кодом донора: у него
		// WriteGap = 60ms в withDefaults, то есть на живой линии тоже. Без
		// паузы два Write уходят в одном такте, и коробка видит поток не так,
		// как видит его браузер, — вердикт про разрез становится вердиктом
		// про нашу спешку.
		o.Gap = 60 * time.Millisecond
	}
