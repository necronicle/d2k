...
// Коды причины отказа команды SET_NAME/SET_ADDR (Event.AckReason, значим
// только при AckOK == false). Обязаны совпадать с D2K_ACK_* в
// datapath/include/d2k_ctl.h.
//
// AckNoRoom выделен из общего отказа НАРОЧНО: замер на живом роутере
// (2026-09-06) поймал ровно то, к чему ведёт отказ без причины — таблица
// планов датапата полна, SET_NAME отвечает отказом, и контроллер (до этой
// правки, onAck в internal/controller/controller.go) читал это как «план
// плохой» и жёг кандидата, которому просто не хватило места. С вытеснением
// по давности в таблице планов (datapath/plans.c) этот код в бою больше не
// приходит — сохранён в протоколе как страховка, а не как ожидаемый исход.
const (
	AckOK      uint8 = 0
	AckBadPlan uint8 = 1
	AckNoRoom  uint8 = 2
...
type Event struct {
	...
	AckOf uint16
	AckOK bool
	// AckReason — почему отказано, значим только при AckOK == false (см.
	// Ack* ниже). Раньше отказ был одним битом, и датапат ответом на
	// SET_NAME/SET_ADDR не мог сказать, отверг ли он план потому, что план
	// негоден, или потому, что в таблице планов не было места — а это два
	// разных факта: место кончилось не по вине кандидата (см. onAck,
	// controller.go). Источник истины — код, а не текст: сравнивать
	// поведение по тексту нельзя (тот же принцип, что у SuspectText).
	AckReason uint8
...
func (c *Conn) Next() (Event, error) {
	...
	case EvAck:
		if len(rest) < 4 {
			return ev, errors.New("подтверждение короче типа команды, признака успеха и причины")
		}
