...
func (c *Controller) Handle(ev control.Event) error {
	...
	case control.EvApplied:
		if t := c.taskForKey(ev.Key); t != nil && t.Current != nil {
			// Applied — «кандидат вообще доехал до какого-то соединения»,
			// это верно для обеих транспортов без оговорок, поэтому считаем
			// всегда.
			c.Applied++
			if ev.Key.Proto == 6 {
				// t.AppliedCount, в отличие от Applied, не просто счётчик:
				// им управляет ротация кандидата (maxSilentTries ниже) по
				// правилу «применили N раз без обмена — кандидат плохой».
				// Правило верно только тогда, когда обмен (EvExchange) в
				// принципе МОГ прийти. У QUIC-потока он прийти не может
				// структурно: session.c считает exchanges/rst_dropped и
				// обходит на молчание (d2k_session_sweep) только TCP-таблицу
				// потоков — handle_udp возвращается раньше этого хвоста
				// d2k_session_packet. Задача 4 научила датапат применять
				// планы и к QUIC-потокам (EvApplied теперь приходит и с
				// Proto==17), но задачи 5/6 (наблюдение и классификация по
				// QUIC) ещё не существуют. Считать сюда QUIC-применения
				// значило бы отбрасывать рабочего TCP-кандидата по молчанию
				// потоков, для которых подтверждение вообще не реализовано —
				// а не потому что кандидат не подошёл. Ревью задачи 4 (круг
				// 2), находка 6: та же задача у TCP и QUIC к одному имени
				// делят один *Task (taskForKey резолвит по имени цели, не по
				// ключу с протоколом), так что без этой оговорки чужой
				// протокол молча портил бы решение о ротации.
				t.AppliedCount++
			}
		}
...
func (c *Controller) onAck(ev control.Event, now time.Time) error {
	...
	if !ev.AckOK {
		if ev.AckReason == control.AckNoRoom {
			// Место в таблице планов датапата кончилось — это НЕ наблюдение
			// о кандидате, и жечь его нельзя: место кончилось не по его вине
			// (замер на живом роутере, 2026-09-06 — до вытеснения по
			// давности в таблице планов датапата, datapath/plans.c, ровно
			// эта путаница сжигала всех кандидатов подряд на каждой цели).
			// Кандидат остаётся текущим: план у него на датапате не встал,
			// но и следующий брать незачем — как только освободится место
			// (вытеснением менее нужной записи датапатом же), тот же план
			// встанет сам при следующей попытке. Если этого не случится,
			// поиск закроется по времени жизни задачи (expire), как любой
			// другой не завершившийся поиск — отдельного таймера под этот
			// случай заводить не нужно.
			c.sayf("по %s датапату не хватило места под %s — кандидат не виноват, жду",
				target, t.Current.Plan.ID)
			return nil
		}
