...
# поэтому FORWARD.
# Обратное направление. Без него в очередь не приходит ни один пакет сервера, и
# датапат слеп на ту сторону: ни подделанного сброса, ни молчания увидеть
# нельзя. Прогон 06.09.2026 стоил трёх попыток именно на этом — «подозрений 0»
# означало «не смотрели», а не «нечего было видеть».
#
...
# означает, что зонд исключён из очереди вместе с посылками, пробный план
# (привязанный к ПОРТУ зонда) не достаётся никому, и рабочий план объявляется
