...
static void car25519(gf o) {
    for (int i = 0; i < 16; i++) {
        int64_t c = o[i] >> 16;
        /* Умножение, а не сдвиг влево: c здесь бывает ОТРИЦАТЕЛЬНЫМ (o[i] со
           знаком), а сдвиг влево отрицательного значения — неопределённое
           поведение (C99 §6.5.7p4). Значение то же самое (умножение на ту же
           степень двойки определено, перенос по построению мал и int64 не
           переполняет), а санитайзер перестаёт быть прав. Поймано первым же
           прогоном настоящего рукопожатия под ASAN/UBSAN (core/test_verify.c):
           до него X25519 не исполнял ни один тест — компилировался, но не
           запускался, и UB жило в дереве незамеченным. */
        o[i] -= c * 65536;
        if (i < 15) {
            o[i + 1] += c;
        } else {
