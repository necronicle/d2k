...
int main(void) {
    /* --- §8: порог живёт на событии (маска seen_types), а не на голом типе
     * записи — зеркало Go HasAppData. Каждый бит — свой тип записи
     * (D2K_TLS_* минус 20); проверяем и «только этот бит», и «этот бит
     * среди прочих», и «все прочие биты, кроме него». ------------------- */
    {
        ...
        CHECK(d2k_ev_has_appdata(&e) == 1, "прикладные данные (один бит) не засчитаны за успех");
        e = ev_with_seen(bit_alert);
        CHECK(d2k_ev_has_appdata(&e) == 0, "предупреждение TLS засчитано за успех — это ОТКАЗ сервера");
        e = ev_with_seen(bit_handshake);
        CHECK(d2k_ev_has_appdata(&e) == 0, "рукопожатие засчитано за успех: §8 требует прикладного обмена");
        e = ev_with_seen(bit_ccs);
        CHECK(d2k_ev_has_appdata(&e) == 0, "смена шифра засчитана за успех");
        e = ev_with_seen(0);
        CHECK(d2k_ev_has_appdata(&e) == 0, "пустая маска засчитана за успех");
        /* Прикладные данные СРЕДИ прочих типов — реалистичный многозаписный
        ...
        CHECK(d2k_ev_has_appdata(&e) == 1, "прикладные данные среди прочих типов не засчитаны за успех");
        /* Все ТРИ прочих типа сразу, но НЕ прикладные данные — граница,
           отличающая «бит appdata» от «маска вообще не нулевая». */
        e = ev_with_seen((uint8_t)(bit_ccs | bit_alert | bit_handshake));
        CHECK(d2k_ev_has_appdata(&e) == 0, "все типы кроме прикладных данных засчитаны за успех");
        e = ev_with_seen(0xFF);
        CHECK(d2k_ev_has_appdata(&e) == 1, "полная маска (включая appdata) не засчитана за успех");
        CHECK(d2k_ev_has_appdata(NULL) == 0, "NULL-событие засчитано за успех");
    }
     ...
     * (22), потом прикладные данные (23) на ОДНОМ потоке. --------------- */
    {
        ...
        CHECK(d2k_ev_has_appdata(&ev1) == 0, "рукопожатие в настоящем обмене засчитано успехом библиотечным порогом");
        ...
        /* Ровно тот вход, на котором старый (удалённый по ревью) подход —
           «успех, если тип ПЕРВОЙ записи == 23» — солгал бы: code второй
           записи EXCHANGE всё равно 22 (липкое поле, проверено строкой
           выше), а не 23. Документируем эту ложь явно, не называя удалённую
           функцию (её больше нет в дереве), а библиотечный порог —
           единственный способ спросить про успех — обязан на ЭТОМ ЖЕ входе
           дать истину. */
        ...
        CHECK(d2k_ev_has_appdata(&ev2) == 1,
              "библиотечный порог не увидел прикладные данные во втором обмене — та самая дырка, которую закрывает seen_types");
    }
