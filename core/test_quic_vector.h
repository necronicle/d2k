/* test_quic_vector.h — один КАПТУРНЫЙ Initial на все тесты ядра, которым он
 * нужен как исходный материал (а не как предмет проверки).
 *
 * Предмет проверки он ровно в одном месте — core/test_quic.c, где сверяется
 * с текстом RFC побайтно, и там он лежит СВОЕЙ копией намеренно: тест разбора
 * не должен зависеть от заголовка, который правит кто-то другой. Здесь же он
 * нужен тем, кто из него ЧТО-ТО СОБИРАЕТ (test_quichello.c) или кому нужен
 * правдоподобный снимок приветствия QUIC (test_sched.c) — для них третья и
 * четвёртая копия тех же 1200 байт была бы просто шумом.
 */
#ifndef D2K_TEST_QUIC_VECTOR_H
#define D2K_TEST_QUIC_VECTOR_H

#include <stdint.h>

/* RFC 9001 Appendix A.2 — клиентский Initial версии 1. ClientHello внутри
 * содержит расширение server_name со значением "example.com". */
static const uint8_t d2k_test_v1_initial[1200] = {
    ,
};

#endif /* D2K_TEST_QUIC_VECTOR_H */
