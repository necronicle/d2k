...
int main(void) {
    ...
    /* --- ТЕЛО ФАЛЬШИВКИ ПОМЕЩАЕТСЯ В КАНАЛ -------------------------------
     *
     * Длина набивки в запасных плечах — самое дорогое число в compose.
     * Лаборатория 13.09.2026 измерила обе крайности: 64 байта перекрывают
     * остаток имени на приветствии зонда и не перекрывают на приветствии
     * браузера (переносимость случайна), а длина приманки без урезания даёт
     * посылку в 1574 байта при канале в 1500 — и вся семья фальшивок
     * перестаёт исполняться. Берётся меньшее из двух, и здесь это проверено. */
    {
        static const size_t caps[] = { 0, 300, 1500 };
        size_t longest[3] = { 0, 0, 0 };
        for (size_t ci = 0; ci < 3; ci++) {
            for (size_t i = 0; i <= (size_t)D2K_FALLBACK_MAX; i++) {
                ...
                const char *line = strstr(text, "payload 1 ");
                if (!line) { continue; }
                line += strlen("payload 1 ");
                const char *end = strchr(line, '\n');
                size_t hex = end ? (size_t)(end - line) : strlen(line);
                CHECK(hex % 2 == 0, "тело фальшивки не парой шестнадцатеричных цифр");
                size_t bytes = hex / 2;
                if (bytes > longest[ci]) { longest[ci] = bytes; }
                if (caps[ci] > 0) {
                    /* Заголовки IPv4 и TCP не длиннее шестидесяти каждый —
                       это ширина их полей длины, а не догадка. */
                    CHECK(bytes + 120 <= caps[ci],
                          "тело фальшивки не помещается в объявленный канал — "
                          "вся семья фальшивок упрётся в отказ отправки");
                }
            }
        }
     ...
     * покрывать лестницу целиком, иначе поиск сдаётся, не дойдя до приёма,
     * который нужен. Замерено на роутере владельца 13.09.2026: по
     * i.ytimg.com поиск кончился на двадцати двух зондах при сорока семи
     * плечах — тридцать одно не пробовалось ни разу.
     *
     * Считаем ПЕРЕБОРОМ, а не повторением формулы: вторая копия арифметики
     * разошлась бы с первой молча, и бюджет снова стал бы меньше списка. */
    {
        char buf[8192];
        size_t last_ok = 0;
        ...
        CHECK(d2k_fallback_arms() == last_ok + 1,
              "объявленное число плеч не сходится с длиной списка — бюджет зондов "
              "окажется меньше лестницы, и поиск сдастся раньше времени");
