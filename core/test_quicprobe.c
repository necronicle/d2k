...
static d2k_tally mock_ask(const char *addr, uint16_t port,
    ...
    /* Мок проверяет ДИСЦИПЛИНУ ДЕРЕВА по содержимому, не сетевые коды
       ...
       (находка 5 ревью, круг 4) проверяется НАСТОЯЩИМИ сокетами в Части 2
       (порт 0 / реальный закрытый порт), не здесь. */
    if (refused_out) {
        *refused_out = 0;
    }
    d2k_tally t;
...
int main(void) {
     ...
     * а не требование стенда. --------------------------------------------- */
    {
        uint32_t rtt_ms = 999;
        int refused = -1;
        d2k_tally t = real_ask("127.0.0.1", 443, NULL, 0, trig_hello(), 150, 0, 99, &rtt_ms, &refused);
     ...
     * СРАЗУ на базовой живости (контроль тоже получает ICMP). Датаграмма
     * ОТПРАВЛЯЕТСЯ успешно (connect()+send() на UDP не видят закрытый порт
     * — проверено эмпирически: ICMP приходит асинхронно, всплывает на
     * recv()/poll() как ECONNREFUSED), значит это ветка "refused > 0", не
     * "не отправилось" — см. соседний тест ниже на порт 0 для второй
     * половины находки 5 ревью (круг 4). --------------------------------- */
    {
        ...
        CHECK(r.verdict == D2K_V_UNREACHABLE,
              "закрытый порт обязан дать UNREACHABLE на базовой живости — честный сетевой отказ");
     ...
     * (находка 5 ревью, круг 4, вторая половина). Порт 0 как адрес назначения
     * UDP — невалидная цель для connect(): ядро отклоняет её СИНХРОННО, ДО
     * выхода на провод (EADDRNOTAVAIL на macOS, эмпирически проверено),
     * значит это ЧИСТЫЙ случай "наша сторона", без единого байта на сети —
     * ноль общего с ICMP-отказом из теста выше, хотя раньше оба давали один
     * и тот же UNREACHABLE. --------------------------------------------- */
    {
        d2k_vres r = d2k_quic_classify("127.0.0.1", 0, "x.example", trig_hello(), ctl_hello(), 0);
        CHECK(r.verdict == D2K_V_FLAKY,
