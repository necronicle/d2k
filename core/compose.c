...
int d2k_fallback_plan(size_t idx, d2k_shape shape, const char *decoy,
    ...
    /* payload 1 — тело фальшивки; payload 2 — приставка перекрытия.
       ...
       НАБИВКА В 64 БАЙТА — ИЗМЕРЕННО ПЛОХОЕ ЧИСЛО, И ОНО ЗДЕСЬ ОСТАЁТСЯ.
       Лаборатория с памятью 13.09.2026 показала: такая фальшивка перекрывает
       остаток имени на приветствии зонда и не перекрывает на приветствии
       браузера — плечо оказывается переносимым СЛУЧАЙНО. Очевидная правка
       (брать длину у приманки, как обещает комментарий ниже) проверена и
       ОТКАЧЕНА: приманка длиной 1534 байта даёт посылку в 1574 байта, а канал
       несёт 1500 — и ВСЯ семья фальшивок перестаёт исполняться вовсе. Это
       хуже: случайная переносимость всё же лечит кого-то, отказ не лечит
       никого.

       Правильный ответ требует того, чего у compose сегодня нет: предела
       отправки. Тело фальшивки обязано покрывать остаток имени И помещаться
       в канал, а оба числа знает датапат. Пока предел не доехал до
       контроллера, число остаётся прежним, и эта запись — не оправдание, а
       описание долга. */
