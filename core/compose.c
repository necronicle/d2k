 ...
 * в 2048 байт вместе с poison/fake/order с большим запасом. Go-сторона
 * (plan.Hello) собирает MODERN-приветствие руками и на буфер такого размера
 * не оглядывается вовсе: там план — не строка ограниченной длины, а срез
 * произвольного размера. Здесь размер задан контрактом задачи 3
 * (char out[][2048]), и это прямое следствие того контракта — см. отчёт
 * задачи 3. */
