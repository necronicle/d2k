...
int d2k_fallback_plan(size_t idx, d2k_shape shape, const char *decoy,
    ...
    uint8_t hello[D2K_COMPOSE_HELLO_MAX];
    size_t hello_len = 0;
    /* Приманка собирается и там, где телом не едет: из неё берётся ДЛИНА
       набивки (см. ниже). Раньше она собиралась только под decoy, и набивка
       молча падала на запасные 64 байта. */
    int need_hello = a.seqovl_hello || a.decoy_hello ||
                     a.badsum || a.repeats > 0 || a.between;
    if (need_hello &&
    ...
    if (have_fake) {
        if (append_fmt(buf, cap, &pos, "payload 1 ") != 0) { return -1; }
        if (a.decoy_hello) {
            /* ТЕЛО ФАЛЬШИВКИ УРЕЗАЕТСЯ ПО КАНАЛУ, и это не «молчаливое
               обрезание фальшивки», которое запрещает U3. Запрет там про
               ДАТАПАТ: он не вправе решать за план, что из него выбросить, уже
               на отправке. Здесь решает СБОРКА — до отправки, зная предел, и
               решение видно в тексте плана.

               Обрезанная приманка остаётся приманкой: её работа — занять
               смещения раньше правды, а не быть разобранной как приветствие.
               Альтернатива — не задавать плечо вовсе — измерена: при канале в
               1500 и приманке в 1534 байта отказ получают ВСЕ плечи с
               приманкой в теле (посылка 1574 байта), то есть семья пропадает
               целиком. */
            size_t room = body_room(send_cap);
            size_t n = (room > 0 && hello_len > room) ? room : hello_len;
            if (append_hex(buf, cap, &pos, hello, n) != 0) { return -1; }
        } else {
            /* ДЛИНА НАБИВКИ — САМОЕ ДОРОГОЕ ЧИСЛО В ЭТОМ ФАЙЛЕ.
               Набивка ВДВОЕ длиннее правды здесь недоступна: длина правды
               ...
               И УРЕЗАЕМ ПО КАНАЛУ. Приманка длиной 1534 байта даёт посылку в
               1574 при канале в 1500, и ВСЯ семья фальшивок перестаёт
               исполняться — измерено лабораторией 13.09.2026, когда набивку
               впервые связали с приманкой. Обратное — набивка в 64 байта —
               измерено там же: она перекрывает остаток имени на приветствии
               зонда и не перекрывает на приветствии браузера, и плечо
               оказывается переносимым СЛУЧАЙНО.
