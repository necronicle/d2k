...
static uint8_t probe_shape(const task *t) {
    ...
    /* ФОРМА ЗОНДА, А НЕ КЛИЕНТА. Здесь стоял разбор снимка КЛИЕНТА, и это
       ...
       НАШ зонд — он ведёт своё рукопожатие TLS 1.3 (core/verify.c), и его
       приветствие всегда современной формы. Когда у клиента форма другая,
       план вставал под клиентскую и к потоку зонда не применялся вовсе:
       «зонд прошёл, а применения этого плана к его потоку не было» — успех
       выброшен. На роутере владельца 13.09.2026 таких выброшенных успехов
       набралось 269.
       Прежде это обходили дедушкиным правом, чтобы план подходил хоть
       чему-нибудь; теперь пробная запись видна только потоку зонда
       (d2k_plans.h), и подгонять её под чужую форму незачем и вредно. */
