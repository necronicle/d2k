    ...
    for (size_t i = 0; i < SCHED_MAX_TASKS; i++) {
        ...
        if (t->state == T_VERIFY_WAIT) {
            if (t->ver_ok) {
                ...
                if (s->dropped_seen != t->ver_dropped0 && t->probes < SCHED_MAX_PROBES) {
                ...
                } else if (t->unsent_code != 0 && refuse_is_permanent(t->unsent_code)) {
                    /* ДЕТЕРМИНИРОВАННЫЙ отказ: тот же план даст тот же
                       результат. Повторять нечего — это причина, по которой
                       опыт НЕВОЗМОЖЕН, а не свойство коробки и не промах
                       кандидата. Берём следующего: иначе одинаковые повторы
                       съедят бюджет, и поиск объявит «рабочего обхода нет»,
                       не проверив ни одного другого плана. */
                    ...
                    t->state = T_PLANNING;
                           ...
                           t->unsent_tries < SCHED_UNSENT_RETRIES) {
                    /* ВРЕМЕННЫЙ отказ: посылка плана не покинула машину из-за
                       всплеска (очередь, ядро). Испытания не было —
                       выбрасывать кандидата не за что, это наша поломка, а не
                       его промах. Испытываем ЕГО ЖЕ ещё раз, тем же приёмом и
                       на тех же основаниях, что и при потере событий выше;
                       повтор ограничен И общим бюджетом зондов, И собственным
                       счётом попыток. */
                    t->unsent_tries++;
                    say(s, "по %s посылка плана не ушла на провод (причина %u, "
                           "попытка %u из %u) — опыта не было, испытываю кандидата ещё раз",
                        t->name, (unsigned)t->unsent_code,
