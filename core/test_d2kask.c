...
static void part_c(void) {
     ...
     * как "спрошен: да" / "ответ: не измерено". ------------------------- */
    {
        ...
        CHECK(contains(r.out, "[4/5]") && contains(r.out, "[5/5]"), "C1: не нашлись заголовки вопросов 4/5");

        /* Без control не задать ОДИН вопрос — разбор протокола, [4/5] в
           донорском порядке. Счёт дубликатов ([5/5]) стал однофакторным и
           берёт от приветствия только длину, поэтому задаётся и без него. */
        {
            const char *p4 = strstr(r.out, "[4/5]");
            const char *p5 = strstr(r.out, "[5/5]");
            CHECK(p4 && p5 && p4 < p5, "C1: вопрос 4 не нашёлся перед вопросом 5");
            CHECK(p4 && strstr(p4, "спрошен: нет") && strstr(p4, "спрошен: нет") < p5,
                  "C1: вопрос про разбор протокола обязан быть НЕ ЗАДАН");
            CHECK(p4 && strstr(p4, "нет control-приветствия") &&
                  strstr(p4, "нет control-приветствия") < p5,
                  "C1: причина непоказана верно для вопроса про разбор протокола");
            CHECK(p5 && strstr(p5, "спрошен: да"),
                  "C1: счёт дубликатов пропущен, хотя control ему больше не нужен");
        }
     ...
     * управляющий сокет — та же причина, что у C2 выше. -------------------- */
    {
        ...
        /* Порядок вопросов донорский (compose.go:33-91): перекрытие, порядок
           сегментов, СУММА, РАЗБОР ПРОТОКОЛА, счёт дубликатов. Значит сумма
           теперь третья, а разбор протокола — четвёртый, и связка «разбор
           пишет оба поля, снимая вопрос про сумму» проверяется на [3/5]
           и [4/5], а не на [4/5] и [5/5]. */
        int outcomes[] = { 0, 0, 0, 1 };
        fakectl_accept_args aa = { lfd, &ps, target_port, outcomes, 4 };
        pthread_t fth;
        CHECK(pthread_create(&fth, NULL, fakectl_accept_and_run, &aa) == 0,
        ...
        if (p3 && p4) {
            CHECK(strstr(p3, "спрошен: да") && strstr(p3, "спрошен: да") < p4,
                  "C3: вопрос про сумму обязан быть ЗАДАН (не пропущен по контролю)");
            CHECK(strstr(p3, "ответ: не измерено") && strstr(p3, "ответ: не измерено") < p4,
                  "C3: вопрос про сумму обязан остаться не измеренным — ответ принадлежит разбору протокола");
            CHECK(strstr(p4, "спрошен: да") != NULL, "C3: вопрос про разбор протокола обязан быть ЗАДАН");
            CHECK(strstr(p4, "ответ: да") != NULL, "C3: вопрос про разбор протокола обязан получить ответ ДА");
            CHECK(strstr(p4, "снят вопрос [3/5]") != NULL,
                  "C3: побочный эффект на вопрос про сумму обязан быть назван явно");
        }
        CHECK(!contains(r.out, "ни один зонд не прошёл"),
