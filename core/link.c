...
int d2k_link_next(int fd, d2k_ev *out, int wait_ms, char *err, size_t errcap) {
    ...
    case D2K_EV_REFUSED:
    default:
        /* Только ключ (REFUSED) либо вид события, которого этот модуль пока
           не разбирает глубже, — событие сообщение, а не обязательство:
           незнакомый вид не повод отказывать, ключ и kind уже разобраны. */
        break;
    }
    return 0;
}
