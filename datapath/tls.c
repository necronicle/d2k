...
int d2k_tls_parse(const uint8_t *b, size_t len, d2k_tls_info *out) {
    ...
    /* ФОРМУ ОБЪЯВЛЯЕМ ТОЛЬКО ПО ПОЛНОМУ БЛОКУ РАСШИРЕНИЙ.
       ...
       Но НАЙДЕННЫЙ признак достоверен и на обрывке: расширение прочитано
       целиком, дальше искать нечего. Поэтому ищем всегда, а решение
       принимает читающий — по паре (is_tls13, exts_truncated). Пропустить
       поиск целиком было бы другой крайностью: у браузера
       supported_versions обычно лежит ДО набивки, то есть в первом же
