...
int main(void) {
    /* --- порядок от старой к новой ---------------------------------------- */
    {
        ...
        for (uint16_t i = 0; i < 5; i++) {
            d2k_key k = mk(i);
            d2k_journal_add(j, 1000 + i, &k, D2K_JRN_HELLO_NONAME, 0, NULL, 0, NULL);
        }
    ...
    /* --- переполнение: старое затирается, потери считаются ------------------ */
    {
        ...
        for (uint16_t i = 0; i < 10; i++) {
            d2k_key k = mk(i);
            d2k_journal_add(j, 2000 + i, &k, D2K_JRN_HELLO_NONAME, 0, NULL, 0, NULL);
        }
    ...
    /* --- обрезка имени ------------------------------------------------------ */
    {
        ...
        d2k_journal_add(j, 1, &k, D2K_JRN_HELLO_SNI, 0,
     ...
     * снимается только то, что ломает печать как таковую.                    */
    {
        ...
        d2k_journal_add(j, 1, &k, D2K_JRN_HELLO_SNI, 0, nasty, sizeof nasty, NULL);
        const d2k_jrn_entry *e = d2k_journal_at(j, 0);
    ...
    /* --- журнал нулевой глубины: законный режим ----------------------------- */
    {
        ...
        d2k_key k = mk(1);
        d2k_journal_add(j, 1, &k, D2K_JRN_HELLO_SNI, 0, (const uint8_t *)"x", 1, NULL);
    ...
    /* --- нулевые аргументы --------------------------------------------------- */
    {
        ...
        d2k_journal_add(NULL, 1, NULL, 0, 0, NULL, 0, NULL);
        ...
        d2k_journal *j = d2k_journal_new(4);
        d2k_journal_add(j, 1, NULL, D2K_JRN_HELLO_NONAME, 0, NULL, 0, NULL);
