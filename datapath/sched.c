...
int d2k_sched_pop_due_serial(d2k_sched *s, uint64_t now_ns,
    ...
    if (s->heap[0].len > outcap) {
        /* БУФЕР ВЫЗЫВАЮЩЕГО МАЛ — И ЭТО ТУПИК, А НЕ ОЖИДАНИЕ.
           Прежняя редакция оставляла такой пакет в очереди, чтобы «не
           исполнить половину плана молча». Но он остаётся В ГОЛОВЕ: очередь
           упорядочена по сроку, и каждая следующая выдача снова упирается в
           него. Одна слишком длинная посылка навсегда запирала ВСЮ очередь,
           и все остальные планы переставали доисполняться.

           Замерено лабораторией 13.09.2026: отложено 19, из очереди ушло 4,
           в очереди на выходе осталось 15, доисполнено 5 из 13 — то есть
           контроллер почти не получал подтверждений, и каждый кандидат
           выглядел неработающим.

           Правильный исход — выбросить ЭТУ посылку и сказать об этом:
           половина плана останется неисполненной в любом случае, но остальные
