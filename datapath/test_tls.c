...
static size_t build_hello_padded(uint8_t *out, const char *sni, size_t pad) {
    ...
    /* supported_versions (43) СРАЗУ ЗА ИМЕНЕМ — как у браузера: признак формы
       лежит в начале блока, а раздувает приветствие набивка в конце. Именно
       поэтому обрыв блока на границе сегмента обычно НЕ мешает узнать форму,
       и пропускать поиск из-за обрыва нельзя. */
    ext[e++] = 0x00; ext[e++] = 0x2b;
    ext[e++] = 0x00; ext[e++] = 0x03;
    ext[e++] = 0x02; ext[e++] = 0x03; ext[e++] = 0x04;
...
int main(void) {
     ...
     * потому, что полная тут не нужна.                                      */
    {
        ...
        /* Признак ищется и на обрывке: у браузера supported_versions обычно
           лежит ДО набивки, то есть в первом же сегменте. Найденный признак
           достоверен — расширение прочитано целиком. Решение принимает
           читающий, по ПАРЕ (is_tls13, exts_truncated): «не нашли И оборвано»
           это «не измерено», а «нашли» — это измерено. */
        CHECK(info.is_tls13,
              "признак TLS 1.3 не найден в первом сегменте, хотя лежит в нём — "
              "поиск пропущен из-за обрыва, и клиент остался бы без обхода");
    }
