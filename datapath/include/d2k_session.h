 ...
 *
 * Число ВЫВЕДЕНО из самого длинного плеча, которое строит составитель, а не
 * выбрано круглым: `fakedsplit-x7` (core/compose.c, g_fb_head — дословно
 * плечо донора) ставит ДВА разреза, то есть три куска нагрузки и два места
 * для фальшивки, и семь копий фальшивки в каждое место: 3 + 2*7 = 17.
 *
 * Прежние 16 были на одну посылку меньше нужного, и это плечо отвергалось
 * целиком на КАЖДОЙ цели — донор его исполняет, а мы нет (0007-donor-parity).
 * Отказ был честным (см. «план описывает больше посылок…» в session.c), но
 * плечо оставалось непроверяемым молча для человека.
 *
 ...
 *
 * По байтам буфера этого хватает с запасом: отправляющий держит OUT_BUF в
 * 32768 при MAX_PKT 1600, то есть 20 полных кадров (d2kd.c). */
#define D2K_RESULT_MAX 17
